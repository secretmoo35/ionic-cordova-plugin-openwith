...
This is a bit modified version of 
[cordova-plugin-openwith](https://github.com/j3k0/cordova-plugin-openwith) by Jean-Christophe Hoelt
for ionic & cordova project
...
export class AppComponent {
  ...
  async initial() {
    ...
    cordova.openwith.init((res: any) => {
      console.log('##### openwith success #####', res);
    }, (err: any) => {
      console.log('##### openwith error #####', err);
    });
  ...
  openwithHandler(intent: any) {
    ...
    for (var i = 0; i < intent.items.length; ++i) {
      ...
      console.log('  name: ', item.name);   // suggested name of the image. For instance: "IMG_0404.JPG"
      console.log('  utis: ', item.utis);   // some optional additional info
      console.log('  fileUrl: ', item.fileUrl);   // some optional additional info
